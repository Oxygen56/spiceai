...
impl DataFusion {
        ...
        let is_ready = match table {
            Table::Accelerated {
                ...
                if let Some(accelerated_table) = accelerated_table {
                    ...
                    let table_provider =
                        self.wrap_policy_table_provider(dataset_table_ref.clone(), table_provider);
                    self.ctx
    ...
    async fn register_accelerated_table(
        ...
        let table_provider = self.wrap_policy_table_provider(dataset.name.clone(), table_provider);
