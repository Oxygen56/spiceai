...
use spice::commands::{
    ...
    install, login, models, nsql, pods, query, refresh, replay, run, search, sql, status, trace,
...
enum Commands {
    ...
    /// Replay a saved chat request JSON file
...
fn run_cli(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Replay(args) => {
            let rt = tokio::runtime::Runtime::new()
                .map_err(|e| spice::error::Error::RuntimeExecution { source: e })?;
            rt.block_on(replay::execute(&ctx, &args))?;
        }
    }
